...
test('Apps Script source loads when review actions precede locale initialization', () => {
  const sourceOrder = ['vendor/Html', 'Config', 'GeminiRouting', 'Digest', 'TextSafety', 'NumericEvidence', 'HtmlEvidence',
    'BytePayload', 'ImageAcquisition', 'CandidateEvidence', 'AIExtraction', 'GmailIdentity', 'SheetSafety', 'SheetState', 'GmailRead',
...
test('only canonical Gmail links provide an identity without partial token collisions', () => {
  ...
  for (const url of ['https://mail.google.com/mail/#all/' + id,
    ...
    'https://mail.google.com/mail/?th=' + id + '&view=pt',
    'https://mail.google.com/mail/?th=' + id + '#',
    'https://mail.google.com/mail/?view=pt&th=' + id + '#']) assert.equal(ctx.sourceId_(url), id);
  for (const url of ['https://MAIL.GOOGLE.COM/mail/#all/' + id,
  ...
  for (const url of ['https://other.example/mail/#all/' + id,
    ...
    'https://mail.google.com/mail/?th=' + id + '#garbage',
    'https://mail.google.com/mail/?th=' + id + '#all/ABCD1234',
    'https://mail.google.com/mail/?th=deadbeef&th=face',
    'https://mail.google.com/mail/?th=' + id + '&th=face#',
...
test('numeric factual evidence cannot be a range or ratio endpoint', () => {
  ...
  for (const source of ['Save 20% in May', 'May specials: 20% off', 'Mayday 20% off', 'Maggioverde 20% off']) {
    assert.equal(numericCandidate('discountValue', '20', source), '20', source);
  }
  for (const source of ['Save 20 Mayé', 'Save 20 Maggioé']) {
    assert.equal(numericCandidate('discountValue', '20', source), '20', source);
  }
  for (const source of ['from 20 through 30 Mayé', 'from 20 through 30 Maggioé', 'from ٢٠ through ٣٠ Mayé']) {
    for (const [low, high] of source.includes('٢٠') ? [['٢٠', '٣٠']] : [['20', '30']]) {
      for (const field of ['discountValue', 'minimumSpend']) {
        assert.equal(numericCandidate(field, low, source), '', field + ': ' + source);
        assert.equal(numericCandidate(field, high, source), '', field + ': ' + source);
      }
    }
  }
  const dateWithOffer = ctx.normalizeCandidate_({merchant: 'Shop', code: 'SAVE20', discountType: '%', discountValue: '20', confidence: 'high', review: false,
    evidence: {merchant: {quote: 'Shop'}, code: {quote: 'SAVE20'}, discountType: {quote: '%'}, discountValue: {quote: '20'}}},
  ...
  for (const [value, text] of [['20', 'Shop SAVE20 20€ off'], ['20', 'Shop SAVE20 20 € off'], ['٢٠', 'Shop SAVE20 ٢٠\u202f€ off']]) {
    const postfix = ctx.normalizeCandidate_({merchant: 'Shop', code: 'SAVE20', discountType: '€', discountValue: value, confidence: 'high', review: false,
      evidence: {merchant: {quote: 'Shop'}, code: {quote: 'SAVE20'}, discountType: {quote: '€'}, discountValue: {quote: value}}},
    {text, images: [], incomplete: false});
  ...
  for (const source of ['save 10% when you spend $20 minimum', 'save 10\u00a0% when you spend $\u202f20 minimum', 'save €10 when you spend $20 minimum', 'save 10€ when you spend $20 minimum']) {
